...
BradescoCartoesMobile.controllers.SeguroCartaoController.home = function(views,params,model){
	...
	_.each(BradescoCartoesMobile.cartoesVisiveis, function(cartao){
		...
		// Caso as informações do seguro ainda não estão disponíveis, consultá-las.
		if (cartao.mostrarSeguroCartao){			
			...
			BradescoCartoesMobile.controller.adapters.consultaCancelamentoSeguroPendente(args).done(function(retornoCancelamentoPend){
				
				cartao.cancelamentoPendente = retornoCancelamentoPend.possuiPedidoPendente;
				cartao.protocoloCancelamento = retornoCancelamentoPend.numeroProtocolo;		
				
					if (cartao.mostrarSeguroCartao && funcionalidade.seguro && cartao.codigoSeguro == 37){
						model.cartoesElegiveis.push(cartao);
					}			
					
					count++;
					
					// Somente mostra a view quando todos os cartões foram checados - evita problemas de sincronismo.		
					if(count == BradescoCartoesMobile.cartoesVisiveis.length) {
						model.cartoesElegiveis.sort(function (a, b){
							return (a.numeroCartao > b.numeroCartao) ? 1 : ((b.numeroCartao > a.numeroCartao) ? -1 : 0); //Funcao para ordenar a lista de cartoes elegiveis
						});	
						views.home(params,model);
					}				
			});
		}
		// Caso o tipo do cartão não tenha seguro (Bradescard) ou funcionalida inativa, somente incrementa o contador.
		else {
			count++;
		}
	});
