cordova.define("scopus-cordova-plugin-calendar.Calendar", function(require, exports, module) { "use strict";
function Calendar() {
}
...
Calendar.prototype.createEventFull = function(events, successCallback, errorCallback) {
  for (var index = 0; index < events.eventListToDelete.length; index++) {
    var event = events.eventListToDelete[index];
    event.startTime = event.startDate instanceof Date ? event.startDate.getTime() : null;
    event.endTime = event.endDate instanceof Date ? event.endDate.getTime() : null;
  }

  for (var index = 0; index < events.eventListToCreate.length; index++) {
    var event = events.eventListToCreate[index];
    event.startTime = event.startDate instanceof Date ? event.startDate.getTime() : null;
    event.endTime = event.endDate instanceof Date ? event.endDate.getTime() : null;
  }
  ...
  array[0] = lstEvents;
  cordova.exec(successCallback, errorCallback, "Calendar", "createEventFull", array);
};
