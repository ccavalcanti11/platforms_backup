(function() {
  ...
  verificaDispositivoSegurancaCadastrado()
    .then(function() {
      ...
      if (AWBE.Platforms.runningOnAndroid()) {
        return $.Deferred().resolve(BradescoCartoesMobile.components.preparaApresentacaoComponentesMToken());
      } else {
        return $.Deferred().resolve(BradescoCartoesMobile.components.validaDispMtokenDirecionamento());
      }
    })
    .fail(function(e) {
  ...
  function appendFormSeguranca() {
    ...
    var formActionName =
      window.injecaoActionName ||
      pageID
        .split("/")
        .pop()
        .replace(/Page$/, "") + "formValidation";
    ...
    function finalizarInjecaoDispSeguranca() {
      ...
      console.log("exibindo div com dispositivo de seguranca");
      $(document).unbind("ajaxComplete");
      $containerDispSeguranca.removeAttr("style");

      var visualizouTutorial = Boolean(AWBE.localStorage.getItem("visualizouTutorial"));
      var  tipoDispositivoSeguranca = AWBE.sessionStorage.getItem("tipoDispositivoConfigurado");
      var fluxoSSO = BradescoCartoesMobile.components.recuperarValorSession("flagSSO");
      if (!visualizouTutorial && !fluxoSSO && tipoDispositivoSeguranca == 3) {
        $("#tutorialAutorizacao").show();
      }

      $(window).scrollTop();
        // declare um callback na view para notificar final do inject
        if (typeof callBackFromInjecaoAfterDone === 'function') {
          console.log('chamando callback afterdone');
          callBackFromInjecaoAfterDone();
        }
  ...
  function verificaDispositivoSegurancaCadastrado() {
    if (window.isInsertingDispositivoSeguranca) {
      return $.Deferred().reject("Dispostivo de seguranca ja esta sendo inserido");
    }
