...
public class Calendar extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (ACTION_OPEN_CALENDAR.equals(action)) {
            ...
            return true;
        } else if (ACTION_CREATE_EVENT_FULL.equals(action)) {
            createEventFull(args);
            return true;
    ...
    private Long findCreateCalendar(String calendarName, String calendarColor){
        JSONArray activeCalendars = null;
        Long calendarId = null;
        if (calendarName == null) {
            callback.error("calendarName is mandatory");
            return null;
        }
        calendarId = Long.getLong(getCalendarAccessor().createCalendar(calendarName, calendarColor));
        try {
            if(calendarId == null){
                Log.d(LOG_TAG, "calendarId is null. " + calendarName + " already exists.");
                activeCalendars = Calendar.this.getCalendarAccessor().getActiveCalendars();
                if(activeCalendars != null && activeCalendars.length() > 0){
                    Log.d(LOG_TAG, "Total of current active calendar: " + activeCalendars.length());

                    String calendarNameFromList = null;
                    JSONObject currentActiveCalendar = null;
                    for(int i = 0; i < activeCalendars.length(); i++) {
                        currentActiveCalendar = activeCalendars.getJSONObject(i);
                        calendarNameFromList = getPossibleNullString("name", currentActiveCalendar);
                        Log.d(LOG_TAG, "Current active calendar: " + calendarNameFromList);

                        if(calendarNameFromList.equalsIgnoreCase(calendarName)){
                            Log.i(LOG_TAG,calendarName + " FOUND.");
                            calendarId = (activeCalendars.getJSONObject(i)).getLong("id");
                            break;
                        }
                    }
                }
            ...
            e.printStackTrace();
            Log.e(LOG_TAG, "Fail to create / find calendar. " + e.getMessage());
            callback.error("Fail to create calendar \"" + calendarName +"\". " + e.getMessage());
        }
        return calendarId;
    }

    private boolean cleanCalendarBeforeInsertion(JSONObject eventObject) {
        try {
            JSONArray eventListToDelete = eventObject.getJSONArray("eventListToDelete");
            if (eventListToDelete != null && eventListToDelete.length() > 0) {
                int deletedEvents = 0;
                for (int i = 0; i < eventListToDelete.length(); i++) {
                    JSONObject eventToDelete = eventListToDelete.getJSONObject(i);
                    boolean deleteResult = getCalendarAccessor().deleteEvent(
                            null, eventToDelete.optLong("startTime"),
                            eventToDelete.optLong("endTime"),
                            getPossibleNullString("title", eventToDelete),
                            getPossibleNullString("location", eventToDelete));
                    if (deleteResult) {
                        deletedEvents++;
                        Log.d(LOG_TAG, "" + getPossibleNullString("title", eventToDelete) + " has been deleted.");
                    } else {
                        Log.e(LOG_TAG, "" + getPossibleNullString("title", eventToDelete) + " has NOT been deleted.");
                    }
                }
                Log.i(LOG_TAG, "Total of events to be deleted: " + eventListToDelete.length());
                Log.i(LOG_TAG, "Events deleted: " + deletedEvents);
            }
        }catch (Exception e){
            Log.e(LOG_TAG, "Fail to delete event. " + e.getMessage());
            e.printStackTrace();
            callback.error("Fail to clean calendar events. " + e.getMessage());
            return false;
        }
        return true;
    }

    private boolean createEventsInBatch(JSONObject eventObject, Long calendarId) {
        try {
            final JSONArray eventListToCreate = eventObject.getJSONArray("eventListToCreate");
            if (eventListToCreate != null && eventListToCreate.length() > 0) {
                int totalEvents = 0;
                for (int i = 0; i < eventListToCreate.length(); i++) {
                    JSONObject eventToRecreate = eventListToCreate.getJSONObject(i);
                    final String createdEventID = getCalendarAccessor().createEvent(
                            null,
                            getPossibleNullString("title", eventToRecreate),
                            eventToRecreate.getLong("startTime"),
                            eventToRecreate.getLong("endTime"),
                            getPossibleNullString("notes", eventToRecreate),
                            getPossibleNullString("location", eventToRecreate),
                            eventObject.optLong("firstReminderMinutes", -1),
                            eventObject.optLong("secondReminderMinutes", -1),
                            getPossibleNullString("recurrence", eventObject),
                            eventObject.optInt("recurrenceInterval"),
                            eventToRecreate.optLong("recurrenceEndTime"),
                            eventToRecreate.optInt("calendarId", calendarId.intValue()),
                            null);
                ...
                Log.i(LOG_TAG, "Total of events to be created: " + eventListToCreate.length());
                Log.i(LOG_TAG, "Events created: " + totalEvents);
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(LOG_TAG, "Fail to create event. " + e.getMessage());
            callback.error("Failed to clean calendar events. " + e.getMessage());
        }
        return false;
    }

    private void createEventFull(JSONArray args) {
        if (args.length() == 0) {
            System.err.println("Exception: No Arguments passed");
            return;
        }
        ...
        try {
            final JSONObject eventObject = args.getJSONObject(0).getJSONObject("events");
            final String calendarColor = getPossibleNullString("calendarColor", eventObject);
            final String calendarName = getPossibleNullString("calendarName", eventObject);
            final JSONArray eventListToCreate = eventObject.getJSONArray("eventListToCreate");
            final JSONArray eventListToDelete = eventObject.getJSONArray("eventListToDelete");
            cordova.getThreadPool().execute(new Runnable() {
                @Override
                public void run() {
                    JSONArray activeCalendars = null;
                    Long calendarId = findCreateCalendar(calendarName, calendarColor);
                    ...
                    if ((calendarId != null)) {
                        if(eventListToDelete != null && eventListToDelete.length() > 0){
                            ...
                            result = cleanCalendarBeforeInsertion(eventObject);
                            if(!result){
                                return;
                            }
                        }
                        ...
                        if(eventListToCreate != null && eventListToCreate.length() > 0){
                            ...
                            result = createEventsInBatch(eventObject, calendarId);
                            if(!result){
                                return;
                            }
                        }
                ...
                public void run() {
                    try {
                        final String createdEventID = getCalendarAccessor().createEvent(
                                ...
                                argObject.optLong("firstReminderMinutes",-1),
                                argObject.optLong("secondReminderMinutes",-1),
