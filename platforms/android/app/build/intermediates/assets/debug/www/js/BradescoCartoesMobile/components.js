...
BradescoCartoesMobile.components.createCarouselHomeLogada= function(flagMostrar, cartoes, $element) {
		...
		if (_.isEmpty(cartoes)) {
			...
			function placeView(idxSlide) {
				...
				if (estaEntreDataCorteEVencimento() && !cartao.bloqueado) {
					BradescoCartoesMobile.controller.adapters.extratoCartao({
						'sessao': sessao,
						...
						if (extrato.statusAberto == 'F' && !jaVisualizouFatura(dataObjUltVcto) && extrato.exibirCardSuaFaturaFechou) {
							var m = {
								cartao: cartao,
								extrato: extrato,
								cpf: cpf,
...
BradescoCartoesMobile.components.carouselPagamento = function($element, viewName, model) {
	...
	var cartoes = BradescoCartoesMobile.cartoesTitular,
		homeCarroussel = false,
		viewDetalhe = AWBE.Views.getView("pagamento/detalhe");
	...
	if(BradescoCartoesMobile.cartaoSelecionado == undefined || BradescoCartoesMobile.cartaoSelecionado >= cartoesPagamento.length) {
		BradescoCartoesMobile.cartaoSelecionado = 0;
	}
	...
	function placeView(idxSlide) {
		...
		card.imagemBase64 = null;
		AWBE.sessionStorage.setItem('meusCartoesAtual', card);
		card.imagemBase64 = tmpImagemBase64;
		
		// evita chamada ao servico de Pagamento se cartao esta bloqueado
		//if (card.bloqueado) {
		//	viewDetalhe.renderTo({}, { codigoRetorno: 0, bloqueado: true }, $target);
		//	return;
		//}
		...
		if (card.formaPagamento == 'F') {
			BradescoCartoesMobile.controller.adapters.recuperarFatura(paramsService).done(function(model) {
				// retira oq que não for numero da linha digitavel
				model.linhaDigitavel = (model.linhaDigitavel) ? model.linhaDigitavel.trim().replace(/[^\d]+/g, '') : '';
        		// faz o merge do model, card e o user --> importante a ordem do model e card, pois o codRetorno deve ser do model ao chegar na view
				viewDetalhe.renderTo({}, _.extend(user, card, model), $target);
			});
		} else {
			// var dataVencCartao = AWBE.sessionStorage.getItem('dataVencCartao'),
			// 	dataVencimento = undefined;

			// if (dataVencCartao && dataVencCartao.cartao && (dataVencCartao.cartao.numeroCartao == card.numeroCartao)) {
			// 	dataVencimento = dataVencCartao.dataVencimento;
			// 	AWBE.sessionStorage.removeItem('dataVencCartao');
			// }
			...
			/** verifcar se data de fechamento e maior que dia de hoje */
			var dataObj = stringDateToJsonObject(card.dataExtrato);

			var createNewModel = function(model) {
				var newModel = {
					formaPagamento: 'D',
					sinalExtrato: model.sinalExtrato,
					totalPagar: model.valorTotal != 'null' ? (model.valorTotal || '0.00') : '0.00',
					pagamentoMinimo: model.valorPagamentoMinimo != 'null' ? (model.valorPagamentoMinimo || '0.00') : '0.00',
			...
			if (dataObj.dataObject < (new Date())) {
				/** data do extrato e maior do que a data autal. Fatura fechada. Podemos proseguir */
				BradescoCartoesMobile.controller.adapters.extratoCartao(params).done(function(model) {
					var newModel = createNewModel(model);
					BradescoCartoesMobile.valorExtratoAberto = model.valorTotal != 'null' ? (model.valorTotal || '0.00') : '0.00';
					viewDetalhe.renderTo(params, newModel, $target);
				});
			} else {
				/** buscamos os meses do extrato */
				BradescoCartoesMobile.controller.adapters.extratoCartao(params).done(function(model) {
					if (model.mesesDisponiveis.length > 1 && model.mesesDisponiveis[1]) {
	...
	var cartaoSelecionado = (BradescoCartoesMobile.cartaoSelecionado == undefined || BradescoCartoesMobile.cartaoSelecionado >= cartoesPagamento.length) ? 0 : BradescoCartoesMobile.cartaoSelecionado;
...
BradescoCartoesMobile.components.validaBadWords = function (inputTexto,successCallback, errorCallBack){
     ...
     BradescoCartoesMobile.controller.adapters.validarBadWords(paramsServico).done(function(response) {
...
BradescoCartoesMobile.components.customBackButton = function () {
	...
	if ($.mobile.activePage.attr('id') === 'cartoes/cadastro/cadastroCartaoSimplificadoPage' && 
			prevPage.id === 'cartoes/adicionarCartoesPage') {
